...
def test_derive_idempotency_key_is_deterministic():
    ...
    first = _derive_idempotency_key(
        ...
        aggregate_id=agg, actor_id=actor, session_id=session, payload=payload,
    )
...
def test_derive_idempotency_key_differs_on_payload_change():
    ...
    first = _derive_idempotency_key(
        ...
        aggregate_id=agg, actor_id=None, session_id=None, payload={"x": 1},
    )
    second = _derive_idempotency_key(
        "crm.interaction_logged",
        aggregate_id=agg, actor_id=None, session_id=None, payload={"x": 2},
    )
    assert first != second
...
def test_infer_interaction_type_lifecycle_changed():
    ...
    assert infer_interaction_type("lead_client.lifecycle_changed", {}) == InteractionType.status_change
    assert infer_interaction_type("consent.updated", {}) == InteractionType.consent_event
    assert infer_interaction_type(
        "training.real_case_logged", {"session_mode": "call"}
    ) == InteractionType.outbound_call
    assert infer_interaction_type("training.real_case_logged", {"session_mode": "chat"}) == InteractionType.note
...
def test_infer_interaction_type_respects_payload_override():
    ...
    assert infer_interaction_type(
        "crm.interaction_logged", {"interaction_type": "meeting"}
...
async def test_emit_domain_event_returns_existing_on_duplicate_key(monkeypatch):
    ...
    monkeypatch.setattr(client_domain.settings, "client_domain_dual_write_enabled", True, raising=False)
...
async def test_emit_domain_event_noop_when_flag_disabled(monkeypatch):
    ...
    monkeypatch.setattr(client_domain.settings, "client_domain_dual_write_enabled", False, raising=False)
    db = SimpleNamespace()
