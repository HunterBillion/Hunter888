...
# ── PR-6: User report about an AI quiz verdict (Variant B) ────────────────
# A user reads the AI's `explanation` for a question, disagrees («it's not
# the law», «AI cited the wrong article»), clicks Flag, fills a reason.
# We persist into knowledge_answer_reports + snapshot the chunks AI cited;
# the methodologist sees it inside KnowledgeReviewQueue under
# `source_kind=user_report` and can flip the linked chunks' status.

from app.schemas.knowledge import AnswerReportRequest, AnswerReportResponse  # noqa: E402
...
async def report_answer(
    ...
    """User flags an AI verdict as wrong (PR-6, Issue: AI «тупит»).
    ...
    if not is_owner:
        # Multi-player: check that user has a participant row in this session
        in_session = (
            await db.execute(
