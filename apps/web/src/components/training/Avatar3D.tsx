...
function InnerGlow({ emotion }: { emotion: string }) {
  ...
  const glowColor = useRef(new THREE.Color(EMOTION_COLORS[emotion] || "#6B7280"));

  useFrame(({ clock }) => {
    if (!meshRef.current) return;
    const t = clock.elapsedTime;
    const target = EMOTION_COLORS[emotion] || "#6B7280";
...
function AvatarMesh({ emotion, isSpeaking, audioLevel }: {
  ...
  // Smooth color transition
  const meshColor = useRef(new THREE.Color(EMOTION_COLORS[emotion] || "#6B7280"));
  const emissiveColor = useRef(new THREE.Color(EMOTION_COLORS[emotion] || "#6B7280"));
...
function GlowRing({ emotion, index = 0 }: { emotion: string; index?: number }) {
  ...
  const ringColor = useRef(new THREE.Color(EMOTION_COLORS[emotion] || "#6B7280"));
  ...
  useFrame(({ clock }) => {
    ...
    lerpColor(ringColor.current, EMOTION_COLORS[emotion] || "#6B7280", 0.03);
...
function Particles({ emotion, count = 100 }: { emotion: string; count?: number }) {
  ...
  const particleColor = useRef(new THREE.Color(EMOTION_COLORS[emotion] || "#6B7280"));
  ...
  useFrame(({ clock }) => {
    ...
    lerpColor(particleColor.current, EMOTION_COLORS[emotion] || "#6B7280", 0.03);
